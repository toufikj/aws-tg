module "eks" {
  ...
  eks_managed_node_groups = {
    for name, config in var.node_groups : name => {
      ...
      subnet_ids      = length(config.subnet_ids) > 0 ? config.subnet_ids : var.subnet_ids
      capacity_type   = config.capacity_type
      spot_max_price  = config.capacity_type == "SPOT" ? config.spot_max_price : null
      
      # Launch template configuration
      launch_template = length(keys(config.launch_template)) > 0 ? {
        id      = try(config.launch_template.id, null)
        name    = try(config.launch_template.name, null)
        version = try(config.launch_template.version, null)
      } : null
